...
**Кнопки в Telegram:** чтобы нажатия по заявкам работали (и статус уходил в leads → Метрику → CSV), у бота должен быть настроен **webhook** на URL функции **telegram-bot-layout**. Без этого Telegram не шлёт в бот события нажатий. У функции **telegram-bot-layout** переменная **LEADS_API_URL** не обязательна (есть дефолт), но если твой leads на другом URL — задай её.
