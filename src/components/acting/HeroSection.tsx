...
export default function HeroSection({ content }: HeroSectionProps) {
  const title = content.acting_hero_title || 'Курсы актёрского мастерства в Москве за 3 месяца!';
  const subtitle = content.acting_hero_subtitle || 'Запишитесь на пробное занятие по актерскому мастерству';
  const description =
    content.acting_hero_description ||
    ...
    <section className="pt-20 pb-32 px-4 md:pt-32 md:pb-40 md:px-4 relative overflow-hidden min-h-[85vh] md:min-h-[90vh] flex items-center">
      ...
      <div className="container mx-auto relative z-10">
        <div className="max-w-4xl lg:max-w-5xl">
          <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-7xl font-bold mb-4 md:mb-6 leading-tight">{title}</h1>
          <p className="text-base sm:text-lg md:text-xl lg:text-2xl text-muted-foreground mb-3 md:mb-4">{subtitle}</p>
          <p className="text-sm sm:text-base md:text-lg text-muted-foreground mb-6 md:mb-8">{description}</p>
